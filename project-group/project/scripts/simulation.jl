...
"""
    check_and_merge!(particles, merge_distance)

Проверка столкновений и слияние частиц при их сближении
Возвращает количество произошедших слияний
"""
function check_and_merge!(particles::Vector{Particle}, merge_distance::Float64)
    n = length(particles)
    merged_count = 0
    
    # Создаем список активных частиц
    active_indices = [i for i in 1:n if particles[i].active]
    
    # Проверяем все пары активных частиц
    ...
    for j in i+1:length(active_indices)
            ...
            # Если частицы перекрываются или очень близко
            if r < merge_distance || r < R_sum
                # Слияние частиц
                new_particle = merge_particles(p1, p2)
                
                # Заменяем p1 на новую частицу, p2 деактивируем
                particles[idx_i] = new_particle
                particles[idx_j].active = false
                merged_count += 1
                
                # Обновляем список активных индексов после слияния
                active_indices = [k for k in 1:n if particles[k].active]
                break  # Выходим из внутреннего цикла, так как индексы изменились
            end
        end
    end
